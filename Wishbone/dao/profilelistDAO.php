...
class profilelistDAO extends abstractDAO
{
    ...
    public function getProfilelists($userid)
    {
        ...
        $getnames = $this->mysqli->query('SELECT firstname,lastname,imagelocation
        	 FROM users
        	 WHERE userid =' . $userid);
        if (! $getnames) {
        ...
        // get the artfromid
        $getartformid = $this->mysqli->query('SELECT artformid 
        	 FROM artist_artform
        ...
        // get the formname
        $getformname = $this->mysqli->query('SELECT formname
        	 FROM artforms
        ...
        $getbio = $this->mysqli->query('SELECT bio,profileid,shareurl,urldes
        	 FROM artprofile
        	 WHERE artistid =' . $artistid);
        if (! $getbio) {
            echo 'Could not run query: ' . mysql_error();
        ...
        $getaddress = $this->mysqli->query('SELECT city,province,country,publicaddress
        	 FROM address
        	 WHERE userid =' . $userid);
        if (! $getaddress) {
            echo 'Could not run query: ' . mysql_error();
        ...
        $getcontact = $this->mysqli->query('SELECT email,phonenumber
        	 FROM contact
        	 WHERE userid =' . $userid);
        if (! $getcontact) {
            echo 'Could not run query: ' . mysql_error();
        ...
        $getexperience = $this->mysqli->query('SELECT experiencetitle, experiencedes,experiencetime
        	 FROM experience
    ...
    public function updateProfilelists($updateinfo, $userid)
    {
        ...
        $getprofileid = $this->mysqli->query('SELECT profileid
        	 FROM artprofile
        	 WHERE artistid =' . $artistid);
        if (! $getprofileid) {
            echo 'Could not run query: ' . mysql_error();
