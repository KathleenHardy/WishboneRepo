...
$query4 = "SELECT availid, availStartDate, availEndDate, availStartTime, availEndTime, availTitle
          FROM entertaineravailability
          WHERE  entid = ? and availid = ?";

if ($stmt4 = $connection->prepare( $query4)) {
    
    $stmt4->bind_param( "ii", $entid, $availid);
    
    //execute statement
    $stmt4->execute();
    
    //bind result variables
    $stmt4->bind_result( $availid, $availStartDate, $availEndDate, $availStartTime, $availEndTime, $availTitle);
...
if ($_SERVER["REQUEST_METHOD"] === "POST" ) {
    ...
    if (isset($_POST['update'])) {
        ...
        $query5 = "UPDATE entertaineravailability
                      SET availTitle = ?, availStartDate = ?, availEndDate = ?, availStartTime = ?, availEndTime = ?
                      WHERE  entid = ? and availid = ?";
        
        if ($stmt5 = $connection->prepare( $query5)) {
            
            $stmt5->bind_param( "sssssii", $availTitle_, $availStartDate_, $availEndDate_, $availStartTime_, $availEndTime_, $entid_, $availid_);
            ...
            if ($status === false) {
                ...
                header("Location: entertainerEventsCalendar.php");
            }
        ...
        $query6 = "DELETE FROM entertaineravailability
                   WHERE  entid = ? and availid = ?";
        
        if ($stmt6 = $connection->prepare( $query6)) {
            
            $stmt6->bind_param( "ii", $entid_, $availid_);
